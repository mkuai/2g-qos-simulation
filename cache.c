...
// 4/17/2015
// Instruction: proposed cache scheme: cache data based on location
...
void init_model(void) /* Initialization function. */
{
	...
	if (add_to_receive(f_ArriveTime, n_ArriveNodeId, n_ArriveDataId) == 0)
	{
		...
		printf("Schedule EVENT_RECEIVE_NEW:\ttime:%f\tnode id:%d\tdata id:%d\n", f_ArriveTime, n_ArriveNodeId,
				n_ArriveDataId);
	}
...
void receive(int n_event_type)
{
	if (list_Receive != NULL)
	{
		...
		// if the event should not happen, schedule a new one
		if (f_Time < sim_time)
			return;
		...
		list_Receive = list_Receive->next; // remove item
		free(old);
...
void schedule_send(void)
{
	...
	if (n_NodeId != -1 && n_DataId != -1 && f_Time > sim_time)
	{
		...
		if (add_to_send(f_Time, n_NodeId, n_DataId) == 0)
		{
			...
			printf("Schedule EVENT_SEND:\ttime:%f\tnode id:%d\tdata id:%d\n", f_Time, n_NodeId,
					n_DataId);
		}
		else
		{
	...
	if (list_Send != NULL)
	{
		...
		while (neighbor != NULL)
		{
			...
			if (add_to_receive(f_ReceiveTime, n_NeighborId, n_DataId) == 0)
			{
				...
				printf("Schedule EVENT_RECEIVE:\ttime:%f\tnode id:%d\tdata id:%d\n", f_ReceiveTime, n_NeighborId,
						n_DataId);
			}
			else
			{
